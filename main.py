...
def save_to_database():
    ...
    sqlForm = "Insert into student (fname ,lname ,fat_fname, fat_lname ,mot_fname  ,mot_lname  ,phn ,email ,per ,course ,username  ,pass ,income,result,imgpath) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    student1 = (
    fName.get(), lName.get(), Fat_fName.get(), Fat_lName.get(), Mot_fName.get(), Mot_lName.get(), phn.get(),
    email.get(), per.get(), Course.get(), set_user.get(), set_pass.get(), income.get(), result,imgpath.get())
...
def adminDashboard():
    ...
    # --------------------variables here we are fetching the data from the database and storing in variables-------------------
        ...
        imgpath = myresult[0][5]
  #---------------------------------------------------------------------------------------------------
        main_root.geometry("1080x470")
        # setting the title
        main_root.title(200 * " " + "Admin Dashboard")
        # adding top frame
        ...
        def HomePage():
                imageFrame=Frame(home_frame,width=100, height=120)
                global photo
                path = "admin/"+imgpath
                img=Image.open(path)
                img=img.resize((150,150),Image.ANTIALIAS)
                photo = ImageTk.PhotoImage(img)
                panel = Label(imageFrame, image=photo)
                panel.pack(side="bottom", fill="both")
                imageFrame.pack(side=LEFT, anchor=NW, padx=30, pady=20)

                """ image=Image.open()
                image=image.resize((150,150),Image.ANTIALIAS)
                photo=ImageTk.PhotoImage(image)
                Label(imageFrame,image=photo).pack()
                imageFrame.pack(side=LEFT, anchor=NW,padx=30,pady=20)"""
...
# -------------------------------------------------contact us--------------
        ...
        def contactUs():
                ...
                # emails
                Label(bodyFrame, text="Email:", font="Arial 15 bold").grid(row=4, column=0)
                Label(bodyFrame, text="shashwatsingh71@gmail.com", font="Arial 11 bold", width=40,
                               anchor=W).grid(row=5, column=1)
                Label(bodyFrame, text="vishnupsingh523@gmail.com", font="Arial 11 bold", width=40,
                               anchor=W).grid(row=6, column=1)
                Label(bodyFrame, text="adityasinghpratham@gmail.com", font="Arial 11 bold", width=40,
                               anchor=W).grid(row=7, column=1)

                # websites
                Label(bodyFrame, text="Websites:", font="Arial 15 bold").grid(row=8, column=0)
                Label(bodyFrame, text="https://linkedin.com/shashwatsingh", font="Arial 11 bold", width=40,
                                 anchor=W).grid(row=9, column=1)
                Label(bodyFrame, text="https://linkedin.com/vishwanathpratapsingh", font="Arial 11 bold",
                                 width=40, anchor=W).grid(row=10, column=1)
                Label(bodyFrame, text="https://linkedin.com/adityasinghpratham", font="Arial 11 bold",
        ...
        home.pack(fill=X, padx=5, pady=40)

        contact = Button(menuTab, text="Contact Us", cursor="hand2", borderwidth=0, command=useContactUs)
        contact.pack(fill=X, padx=5, pady=40)

        profile = Button(menuTab, text="Profile", cursor="hand2", borderwidth=0, command=useAdminDetails)
        profile.pack(fill=X, padx=5, pady=40)


        studentdetail = Button(menuTab, text="Student List", cursor="hand2", borderwidth=0, command=useStudentgetdetail)
        studentdetail.pack(fill=X, padx=5, pady=40)
...
pss=font.Font(size=14,weight="bold")
...
def adminLoginPage():
    ...
    Entry(f_1, textvariable=UniqueID, bg="#ffe05d", fg="black", font="Arial 12 bold").pack(ipady=5, ipadx=5,
                                                                                                 padx=10, pady=10)
    f_1.pack(anchor="w")
    ...
    Entry(f_2, textvariable=passwrd, show="#",bg="#ffe05d", fg="black", font="Arial 12 bold").pack(ipady=5, ipadx=5,
                                                                                                  padx=10, pady=10)
    f_2.pack(anchor="w")
...
def studentDashboard():
    ...
    # --------------------variables here we are fetching the data from the database and storing in variables-------------------
        ...
        studentResult=myresult[0][12]
        studentResult=myresult[0][13]
        studentimg=myresult[0][14]
        # ---------------------------------------------------------------------------------------------------
...
def studentLoginPage():
    ...
    # setting the title
    #main_root.title(100 * " " + "Student LoginPage")

    # -----------------Functions---------------------------------

    # -------------------------------Heading---------------------
    fst1 = Frame(student_login, relief=SUNKEN, bg="black", borderwidth=4)
    Label(fst1, text="Welcome To Rajputana University", bg="Black", fg="white", font="lucida 24 bold").pack()
    fst1.pack(fill="x")

    # -------------------------Body----------------------------
    fstbmain = Frame(student_login, relief=SUNKEN, borderwidth=4, bg="orange", width=900)
    Label(fstbmain, text="Student Credentials", bg="orange", font="Arial 18 bold", fg="white", padx=20, pady=10,
          borderwidth=4).pack()

    f_1 = Frame(fstbmain, borderwidth=8, bg="orange")
    Label(f_1, text="Username", font="comicsansms 12 bold", bg="orange", fg="white").pack(pady=5, side="left")
    Entry(f_1, textvariable=Studentuser, bg="#ffe05d", fg="black", font="Arial 12 bold").pack(ipady=5, ipadx=5,
                                                                                                 padx=10, pady=10)
    f_1.pack(anchor="w")

    f_2 = Frame(fstbmain, borderwidth=8, bg="orange")
    Label(f_2, text="Password", font="comicsansms 12 bold", bg="orange", fg="white").pack(pady=5, side="left")
    Entry(f_2, textvariable=Studentpasswrd, show="#",bg="#ffe05d", fg="black", font="Arial 12 bold").pack(ipady=5, ipadx=5,
                                                                                                  padx=10, pady=10)
    f_2.pack(anchor="w")

    Button(fstbmain, text="Connect", cursor="hand2", bg="#0074D9", fg="white", relief=SUNKEN, font="Arial 12 bold",command= check_pass_student).pack()

    fstbmain.pack(anchor="w", padx=250, pady=20, fill="x")
...
def registrationPage():
    ...
    f_9.grid(row=10, column=0)

    f_10 = Frame(f_m1, borderwidth=8, bg="orange")
    Label(f_10, text="Family Income", font="comicsansms 12 bold", bg="orange", fg="white").grid(row=0, column=2)
    fn = Entry(f_10, textvariable=income, bg="#ffe05d", fg="black", font="Arial 12 bold")
    fn.grid(row=0, column=3, ipady=5, ipadx=5)

    Label(f_10, text="Image Path", font="comicsansms 12 bold", bg="orange", fg="white").grid(row=0, column=5)
    ln = Entry(f_10, textvariable=imgpath, bg="#ffe05d", fg="black", font="Arial 12 bold")
    ln.grid(row=0, column=6, ipady=5, ipadx=5)
    f_10.grid(row=11, column=0)
...
def useStudent_login():
    ...
    Button(student_login, text = "Back to Main",command = backToMain).pack(side = LEFT, anchor = NW)
...
def useAdmin_login():
    ...
    main_root.geometry("840x350")
    Button(admin_login_frame, text = "Back to Main",command = backToMain).pack(side = LEFT, anchor = NE)
...
def useRegistration_Page():
    ...
    Button(main_reg, text = "Back to Main",command = backToMain).pack(side = LEFT, anchor = NE)
